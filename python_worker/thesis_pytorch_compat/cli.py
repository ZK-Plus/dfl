...
def input_data_file(name: str) -> Path:
    candidates = [
        data_dir() / name,
        repo_root() / "neural_network" / "data" / name,
    ]
...
def initial_model_file() -> Path:
    candidates = [
        ...
        data_dir() / "gm.bin",
        data_dir() / "backup.bin",
        repo_root() / "neural_network" / "data" / "random_start.bin",
        repo_root() / "neural_network" / "data" / "backup.bin",
    ]
...
def start_server(
    ...
    try:
        while count < client_limit and not (stop_event and stop_event.is_set()):
            ...
            except Exception as exc:  # noqa: BLE001 - CLI reports and keeps server alive
